...
if __name__ == "__main__":
    ...
    hair_type_woman = [3, 4, 13, 25, 27, 28, 29, 33, 34, 35, 41, 48, 56, 58, 59, 60, 63, 64, 67, 69, 70, 75, 78,
                       79, 80, 82, 86, 87, 90, 95, 96, 102, 6, 7, 9, 36, 40, 42, 47, 55, 57, 65, 68, 71, 81, 84, 85, 93,
                       94, 97, 100,
                       101, 103, 26]
    ...
    idx = (df_cartoon.glasses.isin(glass_type_delete)) | (df_cartoon.hair.isin(hair_type_delete)) | (
        df_cartoon.glasses_color.isin(glass_color_delete)) | (df_cartoon.hair_color.isin(hair_color_delete)) | (
    ...
    idx_women = (
                (df_cartoon_filter1.hair.isin(hair_type_woman)) & (df_cartoon_filter1.facial_hair.isin(facial_no_hair)))
    df_cartoon_filter2 = df_cartoon_filter1.loc[(-df_cartoon_filter1.hair.isin(hair_type_woman)) | (idx_women),
                         :].reset_index(drop=True)

    # Analize images with realistic color of skin and hair
    idx_dark_color = ((df_cartoon_filter2.face_color.isin(face_color_dark)) & (
        -df_cartoon_filter2.hair_color.isin(hair_color_light)))
    df_cartoon_filter_final = df_cartoon_filter2.loc[
                              (-df_cartoon_filter2.face_color.isin(face_color_dark)) | (idx_dark_color), :].reset_index(
        drop=True)

    # save cartoons ids to keep
    df_cartoon_filter_final.to_csv(path_cartoons + "/cartoon100k_limited.csv.gz", header=True, compression="gzip",
                                   index=False)

    # save the png files and then execute th shell scripts
    writePath = "filelist.txt"
    df_cartoon_filter_final["filename_png"] = df_cartoon_filter_final["filename"].apply(
