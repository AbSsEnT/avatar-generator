...
class Transformer(nn.Module):
    def __init__(
        ...
        for (ind, sparse_attn, attn_type, attn_id, ff_id) in \
            ...
            if shift_tokens:
                attn = CachedAs(f'preshift_attn_{ind}',
                                PreShiftToken(attn, image_size=image_fmap_size, seq_len=seq_len))
        ...
        if rotary_emb:
            ...
            img_to_text_freqs = text_pos_emb(
                torch.full((img_seq_len,), 8192))  # image is given a position far away from text
            ...
            img_freqs = broadcat(
                (rearrange(img_freqs_axial, 'i d -> i () d'), rearrange(img_freqs_axial, 'j d -> () j d')), dim=-1)
            img_freqs = rearrange(img_freqs, 'h w d -> (h w) d')

            text_axial_freqs = img_axial_pos_emb(torch.full((text_len,),
                                                            -10.))  # text is given a position of -10 apart from the image axial positions, which is from range [-1, 1]
